...
it('generating diffrence Map Object', ()=>{
    let newBeta = new betaCalc(x,y)
    let diffMap = newBeta.mapifyAll()
    expect(Mapresult).toEqual(diffMap)
})

it('check diffrence map', ()=>{
    let newBeta = new betaCalc(x,y)
    newBeta.mapifyAll()
    let resultMap = newBeta.calcDiff()
    expect(resultMap).toEqual(resultObjectForDiff)
})
